...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        for gt, pred, pred_max_prob in zip(labels, preds_str, preds_max_prob):
            ...
            char_total_cnt += len(pred) + len(gt)
            char_correct_cnt += (len(pred) + len(gt)) * \
                                Levenshtein.ratio(gt,pred)

    accuracy = n_correct / float(length_of_data) * 100
    norm_ED = norm_ED / float(length_of_data)  # ICDAR2019 Normalized Edit Distance
    char_acc = char_correct_cnt / char_total_cnt * 100

    return valid_loss_avg.val(), accuracy, char_acc, preds_str, confidence_score_list, labels, infer_time, length_of_data
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            evaluation_loader = torch.utils.data.DataLoader(
                ...
                num_workers=int(opt.workers),
                collate_fn=AlignCollate_evaluation, pin_memory=True)
            _, accuracy_by_best_model, char_acc, _, _, _, _, _ = validation(
                model, criterion, evaluation_loader, converter, opt)
            log.write(eval_data_log)
            print(f'{accuracy_by_best_model:0.3f}')
            print(f'{char_acc:0.3f}')
            log.write(f'{accuracy_by_best_model:0.3f}\n')
            log.write(f'{char_acc:0.3f}\n')
            log.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_data', required=True, help='path to evaluation dataset',
                        default=r'path to val data')
    parser.add_argument('--benchmark_all_eval', default=False,
                        action='store_true', help='evaluate 10 benchmark evaluation datasets')
    ...
    parser.add_argument('--character',type=str,
                        default=load_characters(),help='character label') # 中文字符集
